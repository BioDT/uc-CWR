...
pasaporte_geoqual_file <- "PasaporteOriginalEvaluadoGEOQUAL.txt"  # Generated by GEOQUAL
...
# We execute TesTable, GEOQUAL, SelecVar, and ELCMapas modules in order
...
source(file.path(param_dpath, "Parameters_TesTable_2021.R"))
ruta <- root_dpath
...
if (TRUE) {  # !!! skip
    ...
    message("- executing")
    source(file.path(tools_dpath, "TestTable.R"))
}

# 2. GEOQUAL
message("GEOQUAL")
source(file.path(param_dpath, "Parameters_GEOQUAL_2020.R"))
ruta <- root_dpath
pasaporte <- pasaporte_file
resultados <- file.path(results_dpath, "GEOQUAL")
dir.create(resultados)
if (file.exists(file.path(resultados, pasaporte_geoqual_file))) {
    message("- skipping")
} else {
    message("- executing")
    source(file.path(tools_dpath, "GEOQUAL.R"))
}

# 3. SelecVar
message("SelecVar")
file.copy(file.path(resultados, pasaporte_geoqual_file), file.path(root_dpath, "Pasaporte", pasaporte_geoqual_file), overwrite=TRUE)
source(file.path(param_dpath, "Parameters_SelecVar_2021.R"))
ruta <- root_dpath
pasaporte <- pasaporte_geoqual_file
totalqual <- 50
pais <- country
resol1 <- resolution
