...
FUN.DownGBIF <- function(species = NULL, # species name as character for whose genus data is to be downloaded
  ...
  #save_ls <- st_write(specs_ls, Species_sf)
  #save_ls <- st_write(specs_ls)
  ...
  saveObj(specs_ls, file = FNAME)
  unlink(occ_get) # removes .zip file
  
  # HJ: section below commented out because it wasn't in use in my tests
...
# Downloading in environmental variable data (NOT READY)
# 1. Bioclimatic: FUN.DownBV (below as in ModGP; not used in my tests, couldn't figure out how)
# 2. Edaphic: FUN.DownEV (missing, just a rough draft)
# 3. Geophysical: FUN.DownGV (missing, just a rough draft)
...
FUN.DownBV <- function(T_Start = 1970, # what year to begin climatology calculation in                       T_End = 2000, # what year to end climatology calculation in
...
# HJ: new part: very rough draft for downloading edaphic and geophysical data
